...
/**
 * Desktop: a compact icon rail fixed to the right edge - per
 * planning brief "consider a compact right-side... CTA rail". Pure server
 * component (just anchors), no client JS. Icons only + a small label that
 * reveals on hover so it never covers page content at rest.
 */
export function StickyConversionRail() {
  return (
    <div className="fixed right-0 top-1/2 z-40 hidden -translate-y-1/2 xl:block">
      <div className="flex flex-col overflow-hidden rounded-l-md border border-r-0 border-border bg-surface-raised shadow-lg">
        <RailItem
        ...
        <RailItem href={telHref()} label="Call Us" gtagEvent="phone_click">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92Z" />
          </svg>
        </RailItem>
        <RailItem href="/contact" label="Get a Quote" gtagEvent="request_quote_click" accent>
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
      ...
      <p className="sr-only">{siteConfig.phone}</p>
    </div>
  );
}
...
function RailItem({
  ...
  return (
    <Comp
      ...
      data-gtag-event={gtagEvent}
      aria-label={label}
      className={`group flex items-center gap-2 border-b border-border px-3.5 py-3.5 text-sm font-medium transition-colors last:border-b-0 ${
        accent ? "bg-brand-signal text-white hover:bg-brand-signal-dark" : "text-ink hover:bg-surface-sunken"
      }`}
    >
      {children}
      <span className="max-w-0 overflow-hidden whitespace-nowrap opacity-0 transition-all duration-150 group-hover:max-w-[9rem] group-hover:opacity-100">
        {label}
      </span>
    </Comp>
  );
